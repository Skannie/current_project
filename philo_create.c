...
/*   Updated: 2022/05/17 21:21:22 by kannie           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_philo(t_waiter *waiter)
{
	...
	{
		if (waiter->must_eat > 0 && f_waiter(waiter) > 0)
			waiter->sig_eat = 1;
		continue ;
	}
	i = -1;
...
int	life(t_philo *philo, long long time)
{
	...
	if (philo->f_kill > 0)
		return (philo->f_kill);
	what_philo_do(philo, time, "32m has taken a forks", 0);
	if (philo->f_kill > 0)
		return (philo->f_kill);
	philo->start = time_to();
	philo_eat(philo, time, "35m is eating");
	if (philo->f_kill > 0)
		return (philo->f_kill);
